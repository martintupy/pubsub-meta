...
class SubscriptionService:
    ...
    def get_subscription(self, live: Optional[Live]) -> Optional[Subscription]:
        subscription = None
        project_id = self._pick_project_id(live)
        if project_id:
            sub = self._pick_subscription(project_id, live)
        if sub:
            subscription = self.client.subscriber_client.get_subscription(subscription=sub)
        return subscription
    ...
    def _pick_subscription(self, project_id: str, live: Optional[Live]) -> Optional[str]:
        subscriptions = []
